...
for(l=0;l<6;l++){
	nfoot = foot[l];
	flagPTP[l] = PTPCollisionDetectOne(&nfoot);
	loopStart[l] = getComputeNeededCellNum(&nfoot,PTP_START);
	loopEnd[l]= getComputeNeededCellNum(&nfoot,PTP_END);
	if((loopStart[l].x>loopEnd[l].x) || (loopStart[l].y>loopEnd[l].y) || (loopStart[l].z>loopEnd[l].z)){
	...
	if(flagPTP[0]==true || flagPTP[1]==true || flagPTP[2]==true || flagPTP[3]==true || flagPTP[4]==true || flagPTP[5]==true){
		for(i=0;i<ptp->n.x;i++){
			for(j=0;j<ptp->n.y;j++){
				for(k=0;k<ptp->n.z;k++){
					cell[i][j][k].CollisionParticleNum=0;
		...
		for(i=0;i<ptp->n.x;i++){
			for(j=0;j<ptp->n.y;j++){
				for(k=0;k<ptp->n.z;k++){
					...
					for(l=0;l<6;l++)
						nfoot = foot[l];
						pLocData = particleCollisionDetect(&nfoot,&particle[i][j][k]);
										
						if(pLocData!=NOT_COLLISION){
						particleWorks(&n,&nfoot,&lastFoot[i],pLocData);
						particle[i][j][k].isCollision = COLLISION;
					    }else if(particle[i][j][k].isCollision != COLLISION){
...
//printf("###########\n");
	...
	//	printf("(%d) cfz=%f sss=%f p=%f a=%f sloc=%f, \n",ptp->lateral, fZCellSize, shearStress_Size, pressure, areaY, ptputil->sqrtVecDP(&lateral_loc)*MM2M);
					...
					case CYLINDER:
						...
						fRDirection =ptputil->normalizeVector(&fRDirection);

						frDirection=ptputil->CrossProduct(&normalDirection, &foot->vel);
						frDirection=ptputil->CrossProduct(&frDirection,&normalDirection);
						frDirection.y = 0;
						frDirection =ptputil->normalizeVector(&frDirection);

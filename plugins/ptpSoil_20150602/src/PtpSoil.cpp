...
void PtpSoil::update(sReal time_ms) {
	...
	//	printf(" %d (%f %f %f) %f\n",num_foot[i], foot[i].vel.x,foot[i].vel.y,foot[i].vel.z,(foot_pos[i].z()-last_pos[i].z()));	
		...
		record_time += time_ms;
	if(!ptpSys->getPTPForce(foot,forceR,forceT)){printf("ERRRROR");}
	else{
		...
		for(int i=0;i<6;i++) {
			if(foot[i].contact != true) FZ[i] = control->nodes->getContactForce(num_foot[i]).z();  
			//else if (foot[i].contact == true && forceR[i].z < -100.f) FZ[i] = -(forceR_last[i].z() + forceR[i].z)/2.f;
			else FZ[i] = -forceR[i].z;	
		}
		FZ_all = FZ[0]+FZ[1]+FZ[2]+FZ[3]+FZ[4]+FZ[5];  
		printf("//%f, %f, %f, %f, %f, %f, [%f]//...\n",FZ[0],FZ[1],FZ[2],FZ[3],FZ[4],FZ[5],FZ_all);
	  ...
	  disp_time = 0;
	    ...
	    msec += step_size*100;
	    fprintf(out,"%f,%f,%f,%f,%f,%f,%f,%f,%f\n",record_time,forceR[1].x,forceR[1].y,forceR[1].z,		
	         control->nodes->getContactForce(num_foot[1]).z(),foot[1].loc.x,foot[1].loc.y,foot[1].loc.z, outF[1].z());

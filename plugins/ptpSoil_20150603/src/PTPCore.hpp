...
//	vecD getOriginalSoilPosition(const vecN n);
	...
	bool calcForce(FOOT* foot, const int type, vecD* forceR, vecD* forceT);
//	int initializePTP(const int p);
	...
	//int ptpWorks(const FOOT (&foott)[6], const FOOT (&lastFoott)[6]);
...
//	int getCellProj(const FOOT* foot, const int type, const int mode);
	...
	PARTICLE particle[MAX_X][MAX_Y][MAX_Z];
	CELL cell[MAX_X][MAX_Y][MAX_Z]; 

...
void PTPUtil::test_class(const vecD i){
printf("PTPUTIL ...vecD = %f\n", i.x);
}
...
vecD PTPUtil::normalizeVector(const vecD* v){
 ...
 double PTPUtil::radianMapper(const double* rad){
	double frct, intg, process;	
	process = *rad;

	frct=modf(*rad/(2.0*PI), &intg);

...
void PtpSoil::drawSoil(PTPSystem* ptpSys){
...
void PtpSoil::createSoil(PTPSystem* ptpSys, vecD* num, vecD* start, vecD* end){
...
//		printf(" %d (%f %f %f)\n",num_foot[i], foot[i].loc.x,foot[i].loc.y,foot[i].loc.z);	
	...
	start0.x = 600;//-2000; //  mm
	start0.y = -700;
	start0.z = -310;// 260;//-310.0;

	end0.x = 1600;//2600;
	end0.y = 700;    
	end0.z = -210;//160;//-210;   
	
	createSoil(ptpSys1, &num0, &start0, &end0); 

	//define foot size and type
	for(int i=0;i<6;i++){
	foot[i].r=25;    //mm
	foot[i].l=0;
	foot[i].d=0;
	foot[i].type = SPHERE;	
      ...
      ptpSys->initializePTP(&num0,&start0,&end0);
...
void PtpSoil::update(sReal time_ms) {
	...
	if(foot_pos[0].x()*M2MM > 700 && readyNextPlane == true ) {
		readyNextPlane = false;
		nextPlane = true;
		...
		start1.x = 800;//foot_pos[5].x()*M2MM - 500;//-2000; //  mm
		start1.y = -700;
		start1.z = -310;// 260;//-310.0;

		end1.x = 1800;//foot_pos[0].x()*M2MM + 500;//1600;//2600;
		end1.y = 700;    
		end1.z = -210;//160;//-210;   

		ptpSys1->initializePTP(&num1,&start1,&end1);
      drawSoil(ptpSys1);
	}
...
//	 if((foot_pos[i].x() > start0.x) && (foot_pos[i].x() < end0.x)) {
		 if(!ptpSys->getPTPForce(foot[i],&forceR[i],&forceT[i])){printf("ERRRROR");}
		 else {
			 wasFirstPlane = true;
			// if(foot[i].contact == true) {		     
...
//				printf(",,,,,%f  -- %f\n", foot_pos[i].z(), outF[i].z());
	...
	//	 }
	 ...
	 //else if (foot_pos[i].x() > start1.x && foot_pos[i].x() < end1.x && !wasFirstPlane) {
		 //if(!ptpSys1->getPTPForce(foot[i],&forceR[i],&forceT[i])){printf("ERRRROR");}
		 //else{ 
			 ////if(foot[i].contact == true) {		     
			 ////	printf("foot collision\n");
			 //outF[i].x() = forceR[i].x;	
...
////				printf(",,,,,%f  -- %f\n", foot_pos[i].z(), outF[i].z());
  	 ...
  	 //}
		...
		//drawSoil();
		  drawSoil(ptpSys);
		 // if(nextPlane == true) drawSoil(ptpSys1);

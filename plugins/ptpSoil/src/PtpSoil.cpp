...
void PtpSoil::init() {
	...
	for(int i=0;i<6;i++) {
		foot[i].loc = control->nodes->getPosition(num_foot[i])*M2MM;
		foot[i].r=25;    
		foot[i].type = SPHERE;	
		...
		num[i].y() = 100;
		num[i].z() = 10;
		start[i].x() = foot[i].loc.x() - 100.0f;
		start[i].y() = foot[i].loc.y() - 100.0f;
		start[i].z() = -310;
		end[i].x() = foot[i].loc.x() + 100.0f;
		end[i].y() = foot[i].loc.y() + 100.0f;
		end[i].z() = -210;

		soil[i] = new PTPInterface(num[i].x(),num[i].y(),num[i].z());
		soil[i]->makePTP(&num[i],&start[i],&end[i]);
	}

 //  	for(int i=0;i<6;i++)  drawSoil(soil[i]);
 
   for(int i=0;i<6;i++) ptplist.push_front( *soil[i] );
   	
   	  list< PTPInterface >::iterator iterEnd = ptplist.end();
  for(list< PTPInterface >::iterator iterPos = ptplist.begin(); 
    iterPos != iterEnd; 
    ++iterPos )
  {
    cout << "아이템 코드 : " << iterPos->testlist << endl;
  }

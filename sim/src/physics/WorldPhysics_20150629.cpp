...
case 34:	foot_id = 0;
	...
	if(first_colliding[foot_id] && obj_pos.x() > 0.6) {
	    ...
	    //SimNode* simnode = control->nodes->getSimNode(geom_data2->id); 	
	    //// geom_data2->id is the object id in plugin   
	...
	if(heightMap_collision[foot_id] && obj_pos.x() > 0.6){
		...
		control->nodes->setFootRadius(foot_id, foot_radius);	    
		 if(!control->nodes->collideOnSoil(foot_id)){printf("ERRRROR\n");}
			 ...
			 Vector outF;
			 double minimumDepth = 0.f;
			control->nodes->getContactForce(foot_id, outF);	
			 //outF.y() = control->nodes->getContactForce(foot_id).y();	
			 //outF.z() = control->nodes->getContactForce(foot_id).z();
			minimumDepth = control->nodes->getMinmumFootDepth(foot_id);
				if(obj_pos.z() < minimumDepth){
				outF.z() = outF.z()+minimumDepth*2200 + obj_vel.z()*100;	
				}
		printf("id = %lu...ouF..(%f %f %f) outF Minmum = %f radius = %f\n", foot_id,
			(dReal)outF.x(),(dReal)outF.y(), (dReal)outF.z(), minimumDepth, foot_radius);				
				if(ABS(outF.z()) <= 150) 
				...
				outF.z() = -150;				
					dBodyAddForce(body1,(dReal)outF.x(), (dReal)outF.y(), -(dReal)outF.z());
				}else 	dBodyAddForce(body1,(dReal)outF.x(), (dReal)outF.y(), -(dReal)outF.z());
...
case 48:		foot_id = 1;
	...
	if(heightMap_collision[foot_id] && obj_pos.x() > 0.6){
		...
		printf("id = %lu, ouF..(%f %f %f) outF \n", foot_id, (dReal)outF.x(),(dReal)outF.y(), (dReal)outF.z());				
				if(ABS(outF.z()) <= 150) 
					dBodyAddForce(body1,(dReal)outF.x(), (dReal)outF.y(), -(dReal)outF.z());
				else if(outF.z() > 150 ) { 
				outF.z() = 150;

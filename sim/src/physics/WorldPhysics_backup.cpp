...
namespace mars {
  namespace sim {
    ...
    WorldPhysics::WorldPhysics(ControlCenter *control) {
      ...
      first_colliding = true;   //ptp
      collideCount = 0;
      prev_collideCount = 0;
      heightMap_collision = false;
      foot_radius = 0;
...
///
	...
	//if(geom_data2->height_field && obj_pos.x() > 0.6) {
////printf("+++++in collide+++ (%d %d) \n", collideCount, prev_collideCount);
		////start to detect PTP collision

	 ////  printf("heightfield...collided %f\n", obj_pos.x());	
				
		////switch(body1){
			////case :	foot_id = 0;
	//if(first_colliding && obj_pos.x() > 0.6) {
		
		//first_colliding = false;
		//heightMap_collision = true;

		////num_foot[6]={34,48,61,73,87,99};		
		//SimNode* simnode1 = control->nodes->getSimNode(34); //with using SC model 
		////SimNode* simnode1 = control->nodes->getSimNode(geom_data2->id-1);  // with using loadScene()
	    //// geom_data2->id-1: nodeID of the sphere 
		//interfaces::NodeData node_data1 = simnode1->getSNode();
		//foot_radius = node_data1.ext.x();

	    //SimNode* simnode = control->nodes->getSimNode(geom_data2->id); 	// geom_data2->id is the object id in plugin   
		//interfaces::NodeData node_data = simnode->getSNode();

		//NodeInterface* node_inter = simnode->getInterface();
        //double height = node_inter->getHeightmapHeight((int)obj_pos.x(), (int)obj_pos.y());
        //printf("testesttes..%f (%f %f %f) id=%lu, radius = %f\n",height,
			//obj_pos.x(),obj_pos.y(),obj_pos.z(), geom_data2->id, foot_radius);
		...
		//num.y() = 100;
		//num.z() = 10;
		//double soildepth = 100*MM2M;
		//start.x() = (obj_pos.x() - foot_radius*2)*M2MM;
		//start.y() = (obj_pos.y() - foot_radius*2)*M2MM;
		////start.x() = foot[foot_id].loc.x() - 100.0f;
		////start.y() = foot[foot_id].loc.y() - 100.0f;		
		//start.z() = (obj_pos.z() - foot_radius - soildepth)*M2MM;//-310;
		//end.x() = (obj_pos.x() + foot_radius*2)*M2MM;
		//end.y() = (obj_pos.y() + foot_radius*2)*M2MM;
		//end.z() = (obj_pos.z() - foot_radius)*M2MM;//-210;
		//PTPInterface* soil = new PTPInterface(num.x(),num.y(),num.z());
		//soil->buildPTP(&num,&start,&end);
		//soil->minPosZ = start.z()*MM2M + soildepth/2.f;
		//control->nodes->pushPTPlist(0,soil);
		//delete soil;
		////printf("+++++in collide+++ (%d %d %d) \n", collideCount, prev_collideCount, soil->testlist);
	//} //첫번째 충돌도 0.5 이상도 아니면 어떻게?.....


	//if(heightMap_collision && obj_pos.x() > 0.6){// && control->nodes->getPTPlist(0,ptplist)){

		//std::list< PTPInterface >::iterator iterEnd = ptplist.end();
		//std::list< PTPInterface >::iterator iter = ptplist.begin(); 
		//if(iter != iterEnd ){	
		
		//Vector outF;			    
	    //iter->vfoot.loc = obj_pos*M2MM;
	    //iter->vfoot.vel = obj_vel*M2MM;
	    //iter->vfoot.r = foot_radius*M2MM;    
		//iter->vfoot.type = SPHERE;	
		
		 //if(!iter->getPTPForce()){printf("ERRRROR");}
		 //else {

			 //outF.x() = iter->contactForce.x();	
			 //outF.y() = iter->contactForce.y();	
				//if(iter->vfoot.loc.z() > iter->minPosZ) outF.z() = iter->contactForce.z();
				//else {
				////outF.z() = soil->contactForce.z() +
					////(soil->vfoot.loc.z()+soil->minPosZ)*2200 + soil->vfoot.vel.z()*100;	
				////}
				////if(ABS(outF.z()) <= 150) 
				...
				//outF.z() = iter->contactForce.z();
				//}
			//}
					//dBodyAddForce(body1,(dReal)outF.x(), (dReal)outF.y(), -(dReal)outF.z());
		//printf("(%f %f %f) collided \n", (dReal)iter->contactForce.x(), 
					//(dReal)iter->contactForce.y(), (dReal)outF.z());

		//printf("(%f %f %f) collided \n", (sReal)body_pos[0],(sReal)body_pos[1],(sReal)body_pos[2]);
		//}
		//return;
	//}

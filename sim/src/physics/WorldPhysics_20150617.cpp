    ...
    if(numc){ 
	  if(geom_data2->height_field) {
		...
		//break;
			//case :	foot_id = 5;
		//break;	
			//default: printf("ERROR: we did not find the foot_id!\n");
		//}	
	    ...
	    ////printf("heightfield...collided \n");	
	    
		//VectorN num[6];  //number of particles in a soil
		//Vector start[6];
		//Vector end[6];

		//foot[foot_id].loc = obj_pos; //control->nodes->getPosition(num_foot[i])*M2MM;
		//foot[foot_id].vel = obj_vel;
		//foot[foot_id].r=25;    
		//foot[foot_id].type = SPHERE;	
		
		//num[foot_id].x() = 100;
		//num[foot_id].y() = 100;
		//num[foot_id].z() = 10;
		//start[foot_id].x() = foot[foot_id].loc.x() - 100.0f;
		//start[foot_id].y() = foot[foot_id].loc.y() - 100.0f;
		//start[foot_id].z() = -310;
		//end[foot_id].x() = foot[foot_id].loc.x() + 100.0f;
		//end[foot_id].y() = foot[foot_id].loc.y() + 100.0f;
		//end[foot_id].z() = -210;

		//soil[foot_id] = new PTPInterface(num[foot_id].x(),num[foot_id].y(),num[foot_id].z());
		//soil[foot_id]->makePTP(&num[i],&start[i],&end[i]);

		//for(int i=0;i<6;i++)  drawSoil(soil[i]);
 
	    
	    //SimNode* simnode = control->nodes->getSimNode(3);    //TODO: attach current NodeId of heightmap
		//NodeInterface* node_inter = simnode->getInterface();
     //   PtpSoil *ptp = node_inter->getPTP();
       // printf("testesttes..%d\n",ptp->test);
      //  ptp->dCollideSoil(&obj_pos, &obj_vel);
        ...
        //p.y() = 0;
        //p.z() = 0;      
		////dBodyAddForceAtPos(body1, 
                           ////(dReal)f.x(), (dReal)f.y(), (dReal)f.z(),
                           ////(dReal)p.x(), (dReal)p.y(), (dReal)p.z());
		//dBodyAddForce(body1,(dReal)f.x(), (dReal)f.y(), (dReal)f.z());
	
		//printf("start...collided \n");		

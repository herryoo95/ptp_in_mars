...
case 34:	foot_id = 0;
	...
	//	   printf("\n geom_data1->id = %lu\n", geom_data1->id);	
		...
		start.x() = (obj_pos.x() - foot_radius*8)*M2MM + 100;
		start.y() = (obj_pos.y() - foot_radius*8)*M2MM;
		start.z() = (obj_pos.z() - foot_radius*2 - soildepth)*M2MM;//-310;
		end.x() = (obj_pos.x() + foot_radius*8)*M2MM +100;
		end.y() = (obj_pos.y() + foot_radius*8)*M2MM;
		end.z() = (obj_pos.z() - foot_radius*2)*M2MM;//-210;
		PTPInterface* soil = new PTPInterface(num.x(),num.y(),num.z());
		soil->buildPTP(&num,&start,&end);
		soil->minPosZ = start.z()*MM2M + soildepth*0.4f;
	...
	if(heightMap_collision[foot_id] && obj_pos.x() > 0.6){
		...
		control->nodes->setFootRadius(foot_id, foot_radius);	    
		 if(!control->nodes->collideOnSoil(foot_id)){printf("ERRRROR\n");}
			...
			double maxForce = 245.f - conforce_all.z();
				//if(obj_pos.z() < minimumDepth){
				//outF.z() = outF.z()- obj_vel.z()*1000 -(obj_pos.z()+minimumDepth)*20000;	
				//}		
		//	printf(".....maxF = %f  outF.z() = %f\n", maxForce, outF.z());						
			if(outF.z() > maxForce) outF.z() = maxForce;
			dBodyAddForce(body1,(dReal)outF.x(), (dReal)outF.y(), (dReal)outF.z());	
					
				//if(obj_pos.z() < minimumDepth){
				//outF.z() = outF.z();//+ obj_vel.z()*100 +(obj_pos.z()+minimumDepth)*500;	
				//}
...
//								(dReal)outF.x(),(dReal)outF.y(), (dReal)outF.z(), minimumDepth, foot_radius);	
		...
		//printf("id= %lu, pos###(%f %f %f) \n", foot_id, (sReal)body_pos[0],(sReal)body_pos[1],(sReal)body_pos[2]);
		}
		//onCalCollision = false;
	 }
break;
   ...
   //   if(numc){ 
    ...
    void WorldPhysics::update(std::vector<draw_item>* drawItems) {
      ...
      if(draw_contact_points) {
		...
		//for(int i=0;i<6;i++) printf("----ContactForce (%f %f %f)\n", conforce[i].x(), conforce[i].y(), conforce[i].z());	
		//printf("----all_conforce (%f %f %f)\n", conforce_all.x(), conforce_all.y(), conforce_all.z());	
